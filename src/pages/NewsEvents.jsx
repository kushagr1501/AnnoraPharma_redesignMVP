          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {newsItems.map((item, idx) => (
              <motion.div key={idx} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} className="group cursor-pointer">
                <div className="bg-white border border-[#1a1a1a]/5 rounded-[24px] overflow-hidden shadow-sm hover:shadow-lg transition-all duration-500 h-full flex flex-col">
                  <div className="h-[240px] w-full relative overflow-hidden">
                    <img src={item.img} alt={item.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                    <div className="absolute top-4 left-4 bg-[#1a1a1a] text-white font-sans text-xs font-bold uppercase tracking-wider px-3 py-1.5 rounded-full">
                      {item.tag}
                    </div>
                  </div>
                  <div className="p-6 flex-1 flex flex-col">
                    <span className="font-sans text-[#1a1a1a]/50 text-xs font-bold tracking-wider uppercase mb-3">{item.date}</span>
                    <h3 className="font-display text-xl text-[#1a1a1a] mb-3 group-hover:text-[#1a1a1a]/70 transition-colors duration-300">{item.title}</h3>
